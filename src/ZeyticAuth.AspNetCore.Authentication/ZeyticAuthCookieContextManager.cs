...
/// <summary>
/// Manage CookieValidatePrincipalContext for ZeyticAuth authentication.
/// It is used to check if the tokens in cookie is expired or not, and refresh using refresh token if needed.
/// </summary>
public class ZeyticAuthCookieContextManager
{
  private readonly string authenticationScheme;
  private readonly CookieValidatePrincipalContext context;
  private ZeyticAuthOptions zeyticOptions
  {
    get => context.HttpContext.RequestServices.GetRequiredService<IOptionsMonitor<ZeyticAuthOptions>>().Get(authenticationScheme);
  }
  ...
  public ZeyticAuthCookieContextManager(
  ...
  public async Task Handle()
  {
    ...
    // the scheme configured for ZeyticAuth authentication.
    ...
    // Until we implement a full-fledged OIDC middleware for ZeyticAuth, we just check for the access token.
    ...
    if (zeyticOptions.Resource != null && !await RefreshTokens(true))
    {
      context.RejectPrincipal();
      return;
    }
  ...
  /// </summary>
  /// <param name="forResource">If it's for the resource specified in the ZeyticAuthOptions.</param>
  ...
  {
    var expiredAtKey = forResource ? ZeyticAuthParameters.Tokens.ExpiresAtForResource : ZeyticAuthParameters.Tokens.ExpiresAt;
    var accessTokenKey = forResource ? ZeyticAuthParameters.Tokens.AccessTokenForResource : ZeyticAuthParameters.Tokens.AccessToken;
    ...
    if (
      !ZeyticAuthUtils.IsExpired(tokenSet.GetValueOrDefault(expiredAtKey))
    )
    {
    ...
    var refreshToken = tokenSet.GetValueOrDefault(ZeyticAuthParameters.Tokens.RefreshToken);
    ...
    ZeyticAuthTokenResponse? tokens;
    ...
    tokenSet[expiredAtKey] = ZeyticAuthUtils.GetExpiresAt(tokens.ExpiresIn);

    if (!string.IsNullOrEmpty(tokens.RefreshToken))
    {
      tokenSet[ZeyticAuthParameters.Tokens.RefreshToken] = tokens.RefreshToken;
    }

    if (!string.IsNullOrEmpty(tokens.IdToken))
    {
      tokenSet[ZeyticAuthParameters.Tokens.IdToken] = tokens.IdToken;
    }
  ...
  /// <summary>
  /// Fetch tokens by making a request to the ZeyticAuth token endpoint with a refresh token.
  /// </summary>
  ...
  public async Task<ZeyticAuthTokenResponse> FetchTokensByRefreshToken(string refreshToken, bool forResource)
  {
    var body = new Dictionary<string, string>
    {
      ["grant_type"] = "refresh_token",
      ["client_id"] = zeyticOptions.AppId,
      ["refresh_token"] = refreshToken,
    };

    if (!string.IsNullOrEmpty(zeyticOptions.AppSecret))
    {
      body["client_secret"] = zeyticOptions.AppSecret;
    }

    if (forResource)
    {
      body["resource"] = zeyticOptions.Resource!;
    }

    // TODO: The token endpoint should be read from the discovery endpoint or the OpenID Connect context.
    var request = new HttpRequestMessage(HttpMethod.Post, $"{zeyticOptions.Endpoint}/oidc/token")
    {
    ...
    return JsonSerializer.Deserialize<ZeyticAuthTokenResponse>(responseString, new JsonSerializerOptions
    {
